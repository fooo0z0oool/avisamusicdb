...
if ($saveOrder && !empty($this->items)) {
	if (Version::MAJOR_VERSION < 4) {
		$saveOrderingUrl = 'index.php?option=com_avisamusicdb&task=albums.saveOrderAjax&tmpl=component';
		$html = HTMLHelper::_('sortablelist.sortable', 'albumList', 'adminForm', strtolower($listDirn), $saveOrderingUrl);
	} else {
		$saveOrderingUrl = 'index.php?option=com_avisamusicdb&task=albums.saveOrderAjax&tmpl=component&' . Session::getFormToken() . '=1';
...
<form action="<?php echo Route::_('index.php?option=com_avisamusicdb&view=albums'); ?>" method="POST" name="adminForm" id="adminForm">
			...
			<table class="table table-striped" id="albumList">
				<thead>
					<tr>
						...
						<th>
							<?php echo HTMLHelper::_('searchtools.sort', 'COM_AVISAMUSICDB_FIELD_ALBUM', 'a.title', $listDirn, $listOrder); ?>
						</th>

						<th>
							<?php echo HTMLHelper::_('searchtools.sort', 'COM_AVISAMUSICDB_FIELD_ACTOR', 'a.actor', $listDirn, $listOrder); ?>
						</th>
						...
						<?php if (JVERSION < 4) : ?>
							...
							<td class="center">
								<?php if (Version::MAJOR_VERSION > 3) { ?>
									<?php echo HTMLHelper::_('jgrid.published', $item->published, $i, 'albums.', true, 'cb'); ?>
								<?php } else { ?>
									<div class="btn-group">
										<?php echo HTMLHelper::_('jgrid.published', $item->published, $i, 'albums.', true, 'cb'); ?>
										...
										<?php
										if ($canChange) {
											HTMLHelper::_('actionsdropdown.' . ((int) $item->published === 2 ? 'un' : '') . 'archive', 'cb' . $i, 'albums');
											HTMLHelper::_('actionsdropdown.' . ((int) $item->published === -2 ? 'un' : '') . 'trash', 'cb' . $i, 'albums');
							...
							<td>
								...
								<?php if ($canEdit) : ?>
									<a class="title" href="<?php echo Route::_('index.php?option=com_avisamusicdb&task=album.edit&id=' . $item->id); ?>">

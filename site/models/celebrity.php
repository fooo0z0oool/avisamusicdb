...
class AvisamusicdbModelCelebrity extends ItemModel
{

	protected $_context = 'com_avisamusicdb.celebrity';
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$this->setState('celebrity.id', $itemId);
	...
	{
		$itemId = (!empty($pk)) ? $pk : (int) $this->getState('celebrity.id');
	...
	{
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('celebrity.id');
		$db = $this->getDbo();

		$db->setQuery(
			'UPDATE #__avisamusicdb_celebrities' .
				' SET hits = hits + 1' .
				' WHERE id = ' . (int) $pk
		);
	}
	...
	/**
	 * Get music list by celebrity ID
	 *
	 * @param int $celebrity_id
	 * @param integer $limit
	 * @return mixed
	 */
	public static function getCelebrityMusicsbyId($celebrity_id, $limit = 4)
	{

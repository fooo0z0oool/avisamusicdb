...
class AvisamusicdbViewCelebrities extends HtmlView
{
    ...
    public function display($tpl = null)
    {
        ...
        // Get model
        $this->total_celebrities = $model->getCountCelebrities()->total_celebrities ?? 0;
        $this->alphabets = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z'];

        foreach ($this->items as &$this->item) {
            $this->item->url = Route::_('index.php?option=com_avisamusicdb&view=celebrity&id=' . $this->item->id . ':' . $this->item->alias . AvisamusicdbHelper::getItemid('celebrities'));
        }
